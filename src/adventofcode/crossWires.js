...
const toCoordinate = line => {
    ...
    let list = line.split(',');
    list.forEach(entry => {
        const to = entry.substr(0, 1)
        const len = entry.substr(1)
        //四个方向
        for (let i = 0; i < len; i++) {
            if (to === 'R') {
                result.push([++x, y])
...
//比较两个坐标数组相同值
const compareCoord = (list1, list2) => {
    let result = [];
...
const getMinD = list => {
    ...
    for (let p of list) {
        const d = Math.abs(p[0]) + Math.abs(p[1]);
        if (d < min) {
            min = d
        }
    }
...
let coorList = compareCoord(
    toCoordinate(line1),
    toCoordinate(line2),
)
let line1List = toCoordinate(line1)
let line2List = toCoordinate(line2)


const findStep = (list, ele) => {
    for(let i = 0; i < list.length; i++) {
        if(list[i][0] === ele[0] && list[i][1] === ele[1]) {
            return i + 1;
        }
    }
}

const calcMinStep = (coorList, list1, list2) => {
    let minStep = Number.MAX_SAFE_INTEGER;

    for(let p of coorList) {
        const step1 = findStep(list1, p);
        const step2 = findStep(list2, p);
        console.log(step1, step2);
        if(step1 + step2 < minStep) minStep = step1 + step2;
    }
...
console.log(calcMinStep(coorList, line1List, line2List));
