...
while (!stable) {
    ...
    for (let i = 0; i < list.length; i++) {
        const rel = list[i].split(')')
        const parent = rel[0]
        const key = rel[1];
        const newVal = nodes.get(parent) + 1;
        if (newVal !== nodes.get(key)) {
            stable = false;
            nodes.set(key, newVal);
        }
    }
}
...
const total = [...nodes.values()].reduce((a, c) => {
    return a + c
})
...
//part 2 找到公共的点，YOU + SAM - 2 * COMMON
...
const buildList = (nodeList) => {
    while (nodeList[nodeList.length - 1] !== 'COM' && nodeList[nodeList.length - 1] !== '') {
        nodeList.push(findParent(nodeList[nodeList.length - 1]))
    }
    return nodeList
}

console.log(buildList(youList));
console.log(buildList(sanList));

const findClosestCommon = (youList, sanList) => {
    for(let youEl of youList) {
        for(let sanEl of sanList) {
            if(youEl === sanEl) {
                return youEl
            }
        }
...
let common = findClosestCommon(youList, sanList)
console.log(common);

//calc min path
//YOU + SAM - COMON
//-2 是为了计算you 与 san 轨道行的距离
let minPath = nodes.get('YOU') + nodes.get('SAN') - 2 * nodes.get(common) - 2
console.log(minPath);
//answer 511